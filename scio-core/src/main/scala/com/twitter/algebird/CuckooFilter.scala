 ...
 * From the inital paper, there is no problem to consider || fingerprint|| = ln(N) where
 * N = fingerprintPerBucket * totalBucket
 * "" as long as we use reasonably sized buckets, the fingerprint size can remain small. "",
 * we'll use a a 32 bits fingerprint.
 *
 * We choose to put a fixed size for the fingerprint.
 *
 * TODO : Make cuckoo filter works like an aggregator like the BloomFilter.
 * TODO : Lookup method have to return a Approximate number like the size method
 * (sometimes you can't insert an element).
 **/
object CuckooFilter {
  def apply[A](fingerprintPerBucket: Int, buckets: Int = 256)(
    implicit hash1: Hash128[A]
  ): CuckooFilterMonoid[A] = CuckooFilterMonoid(fingerprintPerBucket, buckets)
}
...
case class CuckooFilterMonoid[A](fingerprintBucket: Int, totalBuckets: Int = 256)(
  ...
  /**
   * it's come from the BloomFilter. But I don't think it's a good idea because insertion from
   * scratch describe by the "set" and "add" functions are not safe.
   */
   ...
   */
  def create(item: A): CF[A] =
    CFItem[A](new CFHash[A](totalBuckets), fingerprintBucket, totalBuckets, item)
  ...
  def create(data: Iterator[A]): CF[A] =
    sum(data.map(CFItem(cFHash, fingerprintBucket, totalBuckets, _)))
 ...
 * Empty cuckoo filter
 **/
case class CFZero[A](hash: CFHash[A], fingerPrintBucket: Int, totalBuckets: Int = 256)
    extends CF[A] {
  ...
  override def ++(other: CF[A]): CF[A] = {
    ...
    other match {
      ...
      case CFInstance(_, cuckooBit, fpBucket, totalBck) =>
        ...
        CFInstance(
          hash,
          cuckooBitSet.zip(cuckooBit).map(e => e._1.or(e._2)),
          fingerprintBucket,
          bucketNumber
        )
    }
...
case class CuckooFilterAggregator[A](cf: CuckooFilterMonoid[A])
    extends MonoidAggregator[A, CF[A], CF[A]] {
